...
export function programWithFlags<flags, model, msg>(
  ...
  const cmd$ = state$.distinctUntilChanged(cmdCompare).map(state => state[1]).mergeAll()
  const model$ = state$.distinctUntilChanged(modelCompare).map(state => state[0]).share().startWith(initialState[0])
